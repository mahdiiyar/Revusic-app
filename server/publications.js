...
Meteor.publishComposite('reviews.all', function(query, filter, limit) {
  check(query, String);
  check(filter, String);
  check(limit, Number);
  ...
  if (this.userId) {
    ...
    Counts.publish(this, 'reviews.all', Reviews.find(parameters.find), { noReady: true });
    return Reviews.find(parameters.find, parameters.options);
  },
    children: [
      {
        find: (review) => {
        return Meteor.users.find({ _id: review.authorId }, { fields: { emails: 1, username: 1 } });
  }
  }
  ]
  };
